...
`MilestoneGoal` stores one ordered phase under a final goal. The current milestone is derived by repository logic: the first incomplete milestone by `sortOrder` under an active `FinalGoal`.
